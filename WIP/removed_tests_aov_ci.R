if (require("insight") && require("effectsize") && require("testthat") && require("lme4") && require("parameters")) {
  ...
  test_that("model_parameters.aov", {
    ...
    expect_equal(na.omit(mp$Omega2_CI_low), es$CI_low, tolerance = 1e-3)
    expect_equal(mp$Omega2_CI_low, c(0.29018, NA), tolerance = 1e-3)
    expect_equal(na.omit(mp$Omega2_CI_high), es$CI_high, tolerance = 1e-3)
    ...
    expect_equal(na.omit(mp$Eta2_CI_low), es$CI_low, tolerance = 1e-3)
    expect_equal(mp$Eta2_CI_low, c(0.53866, 0, 0, 0, 0, 0, 0, NA), tolerance = 1e-3)
    expect_equal(na.omit(mp$Eta2_CI_high), es$CI_high, tolerance = 1e-3)
  ...
  if (require("car")) {
    ...
    test_that("model_parameters.car-anova", {
      ...
      es <- effectsize::omega_squared(model, partial = TRUE, ci = .9)
      expect_equal(na.omit(mp$Omega2_CI_low), es$CI_low, tolerance = 1e-3)
      expect_equal(mp$Omega2_CI_low, c(0, 0.0284, 0, NA), tolerance = 1e-3)
      expect_equal(na.omit(mp$Omega2_CI_high), es$CI_high, tolerance = 1e-3)
  ...
  test_that("model_parameters.maov", {
    ...
    es <- effectsize::omega_squared(model, partial = TRUE, ci = .9)
    expect_equal(na.omit(mp$Omega2_CI_low), es$CI_low, tolerance = 1e-3)
    expect_equal(mp$Omega2_CI_low, c(0.71218, NA, 0.50841, NA, 0.53774, NA), tolerance = 1e-3)
    expect_equal(na.omit(mp$Omega2_CI_high), es$CI_high, tolerance = 1e-3)
