...
The `model_parameters` function allows you to extract the parameters and their characteristics from various models in a consistent way. It can be considered as a lightweight alternative to [`broom::tidy()`](https://github.com/tidymodels/broom), with some notable differences: 
...
knitr::kable(parameters_table(model_parameters(model)))
```
...
```
```{r, warning=FALSE, message=FALSE, echo=FALSE}
knitr::kable(parameters_table(model_parameters(model, omega_squared = TRUE)))
```
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
model <- glm(vs ~ wt + cyl, data = mtcars, family = "binomial")
knitr::kable(parameters_table(model_parameters(model, standardize = "refit")))
```
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
model <- lm(mpg ~ drat * cyl, data = mtcars)
knitr::kable(parameters_table(model_parameters(model, standardize = FALSE, bootstrap = TRUE)))
```
