...
.omega_squared <- function(model, partial = TRUE) {
  ...
  if (partial == FALSE) {
    params$Omega_Sq <- (params$Sum_Squares - params$DoF * values$Mean_Square_residuals) / (values$Sum_Squares_residuals + values$Mean_Square_residuals)
    params[params$Parameter == "Residuals", "Omega_Sq"] <- NA
  } else {
    params$Omega_Sq_partial <- (params$DoF * (params$Mean_Square - values$Mean_Square_residuals)) / (params$DoF * params$Mean_Square + (values$n - params$DoF) * values$Mean_Square_residuals)
