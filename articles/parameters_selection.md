...
\
`model`` ``<-`` `[`lm`](https://rdrr.io/r/stats/lm.html)`(``Sepal.Length`` ``~`` ``.`` ``*`` ``.``, data ``=`` ``iris``)`\
...
\
[`check_normality`](https://easystats.github.io/performance/reference/check_normality.html)`(``model``)`\
...
[`check_autocorrelation`](https://easystats.github.io/performance/reference/check_autocorrelation.html)`(``model``)`\
`#> OK: Residuals appear to be independent and not autocorrelated (p = 0.574).`\
[`check_collinearity`](https://easystats.github.io/performance/reference/check_collinearity.html)`(``model``)`\
...
[`lm`](https://rdrr.io/r/stats/lm.html)`(``Sepal.Length`` ``~`` ``.`` ``*`` ``.``, data ``=`` ``iris``)`` ``|>`\
`  `[`select_parameters`](https://easystats.github.io/parameters/reference/select_parameters.md)`(``)`` ``|>`\
