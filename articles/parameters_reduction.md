...
\
[`data`](https://rdrr.io/r/utils/data.html)`(``"attitude"``)`\
`model`` ``<-`` `[`lm`](https://rdrr.io/r/stats/lm.html)`(``rating`` ``~`` ``.``, data ``=`` ``attitude``)`\
...
\
`newmodel`` ``<-`` `[`reduce_parameters`](https://easystats.github.io/parameters/reference/reduce_parameters.md)`(``model``)`\
[`parameters`](https://easystats.github.io/parameters/reference/model_parameters.md)`(``newmodel``)`\
`#> Parameter                                                             `\
`#> ----------------------------------------------------------------------`\
`#> (Intercept)                                                           `\
...
\
[`reduce_parameters`](https://easystats.github.io/parameters/reference/reduce_parameters.md)`(``model``, method ``=`` ``"cMDS"``)`` ``%>%`\
...
\
`pca`` ``<-`` `[`principal_components`](https://easystats.github.io/parameters/reference/principal_components.md)`(``insight``::`[`get_predictors`](https://easystats.github.io/insight/reference/get_predictors.html)`(``model``)``, n ``=`` ``"auto"``)`\
`pca`\
...
\
`newdata`` ``<-`` `[`predict`](https://rdrr.io/r/stats/predict.html)`(``pca``)`\
`newdata``$``rating`` ``<-`` ``attitude``$``rating`

We can know update the model with this new component:

\
[`update`](https://rdrr.io/r/stats/update.html)`(``model``, ``rating`` ``~`` ``PC1``, data ``=`` ``newdata``)`` ``%>%`\
`  `[`parameters`](https://easystats.github.io/parameters/reference/model_parameters.md)`(``)`\
...
\
`# Fit the PCA`\
`pca`` ``<-`` `[`model_parameters`](https://easystats.github.io/parameters/reference/model_parameters.md)`(``psych``::`[`principal`](https://rdrr.io/pkg/psych/man/principal.html)`(``attitude``, nfactors ``=`` ``1``)``)`\
`pca`\
