...
\
`# Load the data`\
`data`` ``<-`` ``psych``::`[`bfi`](https://rdrr.io/pkg/psych/man/bfi.html)`[``, ``1``:``25``]`` ``# Select only the 25 first columns corresponding to the items`\
`data`` ``<-`` `[`na.omit`](https://rdrr.io/r/stats/na.fail.html)`(``data``)`` ``# remove missing values`
...
`performance``::`[`check_factorstructure`](https://easystats.github.io/performance/reference/check_factorstructure.html)`(``data``)`\
...
\
`# Fit an EFA`\
`efa`` ``<-`` ``psych``::`[`fa`](https://rdrr.io/pkg/psych/man/fa.html)`(``data``, nfactors ``=`` ``5``)`` ``%>%`\
`  `[`model_parameters`](https://easystats.github.io/parameters/reference/model_parameters.md)`(``sort ``=`` ``TRUE``, threshold ``=`` ``"max"``)`\
\
`efa`\
...
\
`predictions`` ``<-`` `[`predict`](https://rdrr.io/r/stats/predict.html)`(`\
`  ``efa``,`\
`  names ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"Neuroticism"``, ``"Conscientiousness"``, ``"Extraversion"``, ``"Agreeableness"``, ``"Opennness"``)``,`\
`  verbose ``=`` ``FALSE`\
`)`\
`# let's look only at the first five individuals`\
[`head`](https://rdrr.io/r/utils/head.html)`(``predictions``, ``5``)`\
...
\
`n`` ``<-`` `[`n_factors`](https://easystats.github.io/parameters/reference/n_factors.md)`(``data``)`\
`n`\
...
\
[`as.data.frame`](https://rdrr.io/r/base/as.data.frame.html)`(``n``)`\
...
[`summary`](https://rdrr.io/r/base/summary.html)`(``n``)`\
...
\
[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``n``)`` ``+`` `[`theme_modern`](https://easystats.github.io/see/reference/theme_modern.html)`(``)`
...
`partitions`` ``<-`` ``datawizard``::`[`data_partition`](https://easystats.github.io/datawizard/reference/data_partition.html)`(``data``, training_proportion ``=`` ``0.7``, seed ``=`` ``111``)`\
...
\
`structure_big5`` ``<-`` ``psych``::`[`fa`](https://rdrr.io/pkg/psych/man/fa.html)`(``training``, nfactors ``=`` ``5``)`` ``%>%`\
`  `[`efa_to_cfa`](https://easystats.github.io/parameters/reference/convert_efa_to_cfa.md)`(``)`\
`structure_big6`` ``<-`` ``psych``::`[`fa`](https://rdrr.io/pkg/psych/man/fa.html)`(``training``, nfactors ``=`` ``6``)`` ``%>%`\
`  `[`efa_to_cfa`](https://easystats.github.io/parameters/reference/convert_efa_to_cfa.md)`(``)`\
\
...
[`library`](https://rdrr.io/r/base/library.html)`(`[`performance`](https://easystats.github.io/performance/)`)`\
\
`big5`` ``<-`` `[`suppressWarnings`](https://rdrr.io/r/base/warning.html)`(``lavaan``::`[`cfa`](https://rdrr.io/pkg/lavaan/man/cfa.html)`(``structure_big5``, data ``=`` ``test``)``)`\
`big6`` ``<-`` `[`suppressWarnings`](https://rdrr.io/r/base/warning.html)`(``lavaan``::`[`cfa`](https://rdrr.io/pkg/lavaan/man/cfa.html)`(``structure_big6``, data ``=`` ``test``)``)`\
\
`performance``::`[`compare_performance`](https://easystats.github.io/performance/reference/compare_performance.html)`(``big5``, ``big6``, verbose ``=`` ``FALSE``)`\
...
`#> ---------------------------------------------------------------------`\
`#> big5 | 0.884 | 0.831 | 0.748 | 0.762 | 0.788 | 0.071 | [0.068, 0.075]`\
`#> big6 | 0.876 | 0.820 | 0.722 | 0.727 | 0.760 | 0.077 | [0.073, 0.080]`\
`#> `\
...
`M`` ``<-`` ``0.5`` ``*`` ``X`` ``+`` `[`rnorm`](https://rdrr.io/r/stats/Normal.html)`(``100``)`\
`Y`` ``<-`` ``0.7`` ``*`` ``M`` ``+`` `[`rnorm`](https://rdrr.io/r/stats/Normal.html)`(``100``)`\
`df`` ``<-`` `[`data.frame`](https://rdrr.io/r/base/data.frame.html)`(``X ``=`` ``X``, Y ``=`` ``Y``, M ``=`` ``M``)`\
\
`model`` ``<-`` ``" # direct effect`\
`             Y ~ c*X`\
`           # mediator`\
...
`             total := c + (a*b)`\
`         "`\
`fit`` ``<-`` ``lavaan``::`[`sem`](https://rdrr.io/pkg/lavaan/man/sem.html)`(``model``, data ``=`` ``df``, test ``=`` ``"Satorra-Bentler"``)`\
