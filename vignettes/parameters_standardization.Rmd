...
comparison <- function(model, robust=FALSE){
  out <- standardize_parameters(model, method="refit", robust=robust)
  out$full <- out$Std_Estimate - standardize_parameters(model, method="full", robust=robust)$Std_Estimate
  out$classic <- out$Std_Estimate - standardize_parameters(model, method="classic", robust=robust)$Std_Estimate

  # out$MuMin <- out$estimate - MuMIn::std.coef(model, partial.sd=FALSE)[, 1]
  out$sjstats <- out$Std_Estimate - c(NA, sjstats::std_beta(model)[, 2])
  out$lm.beta <- out$Std_Estimate - lm.beta::lm.beta(model)$standardized.coefficients
  out$Std_Estimate <- NULL
  return(out)
}
```


## Simple models with only numeric predictors
