...
comparison <- function(model, robust=FALSE){
  ...
  out$full <- tryCatch({
    out$Std_Estimate - standardize_parameters(model, method="full", robust=robust)$Std_Estimate
}, error = function(error_condition) {
    "Error"
})
  out$classic <- tryCatch({
    out$Std_Estimate - standardize_parameters(model, method="classic", robust=robust)$Std_Estimate
}, error = function(error_condition) {
    "Error"
})
  out$sjstats <- tryCatch({
    out$Std_Estimate - c(NA, sjstats::std_beta(model)[, 2])
}, error = function(error_condition) {
    "Error"
})
  
  out$lm.beta <- tryCatch({
    out$Std_Estimate - lm.beta::lm.beta(model)$standardized.coefficients
}, error = function(error_condition) {
    "Error"
})
