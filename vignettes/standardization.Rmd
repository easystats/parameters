...
output: 
  ...
  rmarkdown::html_vignette:
    ...
    fig_width: 10.08
    fig_height: 6
tags: [r, parameters, standardization, effect size, cohen d, standardized coefficients]
vignette: >
  %\VignetteIndexEntry{Standardized parameters}
...
```
```{r message=FALSE, warning=FALSE, echo=FALSE}
knitr::kable(parameters_standardize(model, method="full"), digits=2)
```
...
comparison <- function(model, robust=FALSE){
  ...
  out$full <- tryCatch({
    out[, 2] - parameters_standardize(model, method="full", robust=robust)[, 2]
  ...
  out$sjstats <- tryCatch({
    if(insight::model_info(model)$is_bayesian){
      out[, 2] - c(NA, bayestestR::describe_posterior(as.data.frame(sjstats::std_beta(model)))[, 2])
    } else{
