...
However, we can use a workaround by using `pool_parameters()`, which works on a list of `parameters_model` objects (as returned by `model_parameters()`). So whenever a model-object is not yet supported by `mice::with()`, you can instead fit multiple models to the imputed datasets, call `model_parameters()` for each model and then pool all parameters with `pool_parameters()`:

The steps would be:

1. calculating the regression models for each imputed dataset manually, using `complete()` from package *mice* to get the imputed datasets.
2. for each model, we call `model_parameters()` and save the results in a list.
3. this list is then passed to `pool_parameters()`. 
