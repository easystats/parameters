...
This returns as the coefficient of the "within"-effects, which is `1.2`, with a standard error of `0.07`. Note that the FE-model does *not* take the variation *within* subjects into account, thus resulting in (possibly) slightly biased estimates, and biased standard errors.
...
Finally, we can also take the variation between subjects into account by adding a random slope. This model can be calle a complex "REWB" (random-effects within-between) model. Due to the variation between subjects, we get larger standard errors for the within-effect.
