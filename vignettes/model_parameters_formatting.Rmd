...
`to_table()` allows to format the output of functions like `model_parameters()` or `simulate_parameters()` in markdown-format, to create pretty rendered tables inside markdown documents. These tables are still pretty formatted when knitting markdown-documents into Word or PDF.
