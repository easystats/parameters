...
When the parameters table also includes degrees of freedom, and the degrees of freedom are the same for each parameter, then this information is included in the statistic-column. This is usuall the default for `model_parameters()`:
...
## Formatting the Parameters Table

Finally, `format_table()` from *insight* formats data frame and returns a character vector that can be printed to the console or inside rmarkdown documents. The data frame then looks more "table-like".

```{r}
data(mtcars)
cat(format_table(mtcars[1:8, 1:5]))
```
...
model %>% 
  ...
  format_table(format = "markdown", caption = "My Table", align = "lcccrr")
```

`to_table()` is a convenient wrapper around `parameters_table()` and `format_table(format = "markdown")`, and allows to directly format the output of functions like `model_parameters()`, `simulate_parameters()` or other _parameters_ functions in markdown-format.

These tables are also nicely formatted when knitting markdown-documents into Word or PDF. `to_table()` applies some default settings that have proven to work well for markdown, PDF or Word tables.

```{r}
model_parameters(model) %>% to_table()
```

`to_table()` is considered as the main-function for users who want to generate nicely rendered tables inside markdown-documents.
