...
The `style` argument can be used for a more customized output. See examples below for `compare_parameters()`.
...
The `style` argument also has a basic support for glue-like syntax, which lets you layout model elements in a very flexible way. Following tokens are replaced by the related summary coefficients or statistics:
...
compare_parameters(lm1, lm2, lm3, style = "{estimate}{stars} ({se})")

# estimates, CI, p and stars
compare_parameters(lm1, lm2, lm3, style = "{estimate} ({ci_low}, {ci_high}), p={p}{stars}")
```

`style` also works for `model_parameters()`, however, it's necessary to call this argument via the `print()` method:

```{r}
# estimates, p-stars and CI in parentheses
result <- model_parameters(lm3)
print(result, style = "{estimate}{stars} ({ci})")
```
