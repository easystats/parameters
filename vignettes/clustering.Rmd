...
Note that in order to fully use all the methods demonstrated below, you will need to additionnaly install the packages below:

```{r eval = FALSE}
install.packages(c("NbClust", "mclust", "pvclust", "cluster", 
                   "fpc", "dbscan", "mclust"))
```
...
Hierachical DBSCAN is a variant that does not require the critical **EPS** argument. It computes the hierarchy of all DBSCAN solutions, and then finds the optimal cuts in the hierarchy using a stability-based extraction method.
