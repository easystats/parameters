...
>
> Instead of
> [`library(parameters)`](https://easystats.github.io/parameters/), use
...
![](reference/figures/figure1.png)

The
[`model_parameters()`](https://easystats.github.io/parameters/articles/model_parameters.html)
function (that can be accessed via the
[`parameters()`](https://easystats.github.io/parameters/reference/model_parameters.md)
shortcut) allows you to extract the parameters and their characteristics
from various models in a consistent way. It can be considered as a
...
\
`model`` ``<-`` `[`lm`](https://rdrr.io/r/stats/lm.html)`(``Sepal.Width`` ``~`` ``Petal.Length`` ``*`` ``Species`` ``+`` ``Petal.Width``, data ``=`` ``iris``)`\
\
...
[`model_parameters`](https://easystats.github.io/parameters/reference/model_parameters.md)`(``model``, standardize ``=`` ``"refit"``)`\
`#> Parameter                           | Coefficient |   SE |         95% CI | t(143) |      p`\
`#> -------------------------------------------------------------------------------------------`\
...
[`model_parameters`](https://easystats.github.io/parameters/reference/model_parameters.md)`(``model``, vcov ``=`` ``"HC3"``)`\
`#> Parameter                           | Coefficient |   SE |         95% CI | t(143) |      p`\
`#> -------------------------------------------------------------------------------------------`\
...
`model`` ``<-`` `[`lmer`](https://rdrr.io/pkg/lme4/man/lmer.html)`(``Sepal.Width`` ``~`` ``Petal.Length`` ``+`` ``(``1`` ``|`` ``Species``)``, data ``=`` ``iris``)`\
\
...
[`model_parameters`](https://easystats.github.io/parameters/reference/model_parameters.md)`(``model``, ci_method ``=`` ``"kenward"``, effects ``=`` ``"fixed"``)`\
`#> # Fixed Effects`\
`#> `\
...
\
`model`` ``<-`` ``psych``::`[`fa`](https://rdrr.io/pkg/psych/man/fa.html)`(``attitude``, nfactors ``=`` ``3``)`\
[`model_parameters`](https://easystats.github.io/parameters/reference/model_parameters.md)`(``model``)`\
...
\
[`lm`](https://rdrr.io/r/stats/lm.html)`(``disp`` ``~`` ``.``, data ``=`` ``mtcars``)`` ``|>`\
...
- the `s_value` argument (setting `s_value = TRUE`) in
  [`model_parameters()`](https://easystats.github.io/parameters/reference/model_parameters.md)
  replaces the p-values with their related *S*-values (@ Rafi &
  Greenland, 2020)
