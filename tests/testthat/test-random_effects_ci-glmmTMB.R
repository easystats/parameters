...
if (FALSE &&
  ...
  m1 <- glmmTMB(angle ~ temperature + (temperature | recipe) + (temperature | replicate), data = cake)
  m2 <- glmmTMB(Reaction ~ Days + (Days | Subject), data = sleepstudy)
  m3 <- glmmTMB(angle ~ temperature + (temperature | recipe), data = cake)
  m4 <- glmmTMB(angle ~ temperature + (temperature | replicate), data = cake)
  m5 <- glmmTMB(Reaction ~ Days + (Days + Months | Subject), data = sleepstudy)

  mp1 <- model_parameters(m1) # fails
  mp2 <- model_parameters(m2) # works
  mp3 <- model_parameters(m3) # no SE/CI
  mp4 <- model_parameters(m4) # no SE/CI
  mp5 <- model_parameters(m5) # no SE/CI
  ...
  m5 <- glmmTMB(Reaction ~ Days + (0 + Days + Months | Subject), data = sleepstudy)
