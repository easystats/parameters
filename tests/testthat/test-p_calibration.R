...
test_that("p_calibration model", {
  expect_silent(p_calibration(model, verbose = FALSE))
  expect_warning(out <- p_calibration(model))
...
test_that("p_calibration numeric", {
  ...
  out <- p_calibration(p, type = "bayes")
  expect_equal(out, c(0.875, 0.6259, 0.4072, 0.1252, 0.072, 0.0188), tolerance = 1e-3)
})


test_that("p_calibration print", {
  out <- p_calibration(model, verbose = FALSE)
