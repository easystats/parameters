...
test_that("model_parameters.mixed", {
  params <- model_parameters(circus::lmerMod_1, standardize = TRUE)
  testthat::expect_equal(c(nrow(params), ncol(params)), c(2, 9))

  params <- model_parameters(circus::merMod_1, standardize = TRUE)
  testthat::expect_equal(c(nrow(params), ncol(params)), c(2, 9))

  params <- model_parameters(circus::merMod_2, standardize = TRUE)
  testthat::expect_equal(c(nrow(params), ncol(params)), c(2, 9))
