...
test_that("model_parameters, robust", {
  params1 <- model_parameters(model, standardize = "refit", robust = TRUE)
  params2 <- model_parameters(model, standardize = "refit", vcov_estimation = "HC3")
  robust_se <- unname(sqrt(diag(sandwich::vcovHC(model2))))
  expect_equal(params1$SE, robust_se, tolerance = 1e-3)
  expect_equal(params1$p, c(0.28624, 0.0259, 0.43611, 0.01197, 0.95238, 0.01165), tolerance = 1e-3)
  expect_equal(params2$SE, robust_se, tolerance = 1e-3)
  expect_equal(params2$p, c(0.28624, 0.0259, 0.43611, 0.01197, 0.95238, 0.01165), tolerance = 1e-3)
})
