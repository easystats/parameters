if (require("testthat") && require("parameters") && require("insight") && require("effectsize") && require("lme4")) {
  ...
  t <- format_table(x)
  expect_true(all(names(t) == c("Parameter", "Coefficient", "SE", "CI", "95% CI", "t(147)", "p")))

  x <- model_parameters(lme4::lmer(Sepal.Length ~ Petal.Length + (1 | Species), data = iris))
  t <- format_table(x)
  expect_true(all(names(t) == c("Parameter", "Coefficient", "SE", "CI", "95% CI", "t(146)", "p")))
  ...
  t <- format_table(x)
  expect_true(all(names(t) == c("Parameter", "Std. Coef.", "SE", "CI", "95% CI", "t(147)", "p")))
