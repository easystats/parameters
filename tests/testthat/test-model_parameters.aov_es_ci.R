# these are to be run only locally and neither on CRAN nor on CI platforms
...
&& require("car") && require("gam")) {
  ...
  test_that("works with aovlist", {
    ...
    testthat::expect_equal(
      ...
      structure(list(
        Group = c(
          "block", "block", "Within", "Within",
          "Within", "Within", "Within", "Within", "Within"
        ), Parameter = c(
          "N:P:K",
          "Residuals", "N", "P", "K", "N:P", "N:K", "P:K", "Residuals"
        ),
        Sum_Squares = c(
          37.0016666666667, 306.293333333333, 189.281666666667,
          8.40166666666664, 95.2016666666666, 21.2816666666667, 33.135,
          0.481666666666667, 185.286666666667
        ), df = c(
          1, 4, 1, 1,
          1, 1, 1, 1, 12
        ), Mean_Square = c(
          37.0016666666667, 76.5733333333333,
          189.281666666667, 8.40166666666664, 95.2016666666666, 21.2816666666667,
          33.135, 0.481666666666667, 15.4405555555556
        ), F = c(
          0.483218701027338,
          NA, 12.2587342136509, 0.544129816860359, 6.16568920231712,
          1.37829669341201, 2.14597200733998, 0.0311949051919548, NA
        ), p = c(
          0.525236141197407, NA, 0.00437181182579937, 0.474904092674435,
          0.0287950535002327, 0.263165282877168, 0.168647878500492,
          0.862752085685407, NA
        ), Omega2_partial = c(
          -0.0942477999055258,
          NA, 0.229512910165375, -0.0122085297827422, 0.120238953669017,
          0.00990967810040318, 0.0294275532444687, -0.0263066622506334,
          NA
        ), Omega2_CI_low = c(0, NA, 0, 0, 0, 0, 0, 0, NA), Omega2_CI_high = c(
          0,
          NA, 0.521347896232783, 0, 0.421019071177616, 0.0219773797217655,
          0.291675633173735, 0, NA
        ), Eta2 = c(
          0.0422217531127632, NA,
          0.215984968211495, 0.00958694912127554, 0.108632438158378,
          0.0242840216880714, 0.0378095884705574, 0.000549618785171324,
          NA
        ), Eta2_CI_low = c(0, NA, 0, 0, 0, 0, 0, 0, NA), Eta2_CI_high = c(
          0.492138468881537,
          NA, 0.510212521273883, 0.22616716369881, 0.408375708137791,
          0.278952604688375, 0.309385191526976, 0.0381674766826207,
          NA
        )
      ), row.names = c(NA, -9L), class = "data.frame", ci = 0.9, model_class = c(
        "aovlist",
        "listof"
      ), digits = 2, ci_digits = 2, p_digits = 3),
      tolerance = 0.001
  ...
  test_that("works with anova", {
    ...
    testthat::expect_equal(
      ...
      structure(list(Parameter = c(
        ...
        11.6147000439174, 212.213777777778,
        175.488927849928, 817.763961038961
      ), df = c(2, 1, 2, 39), Mean_Square = c(
        5.80735002195871,
        212.213777777778, 87.7444639249639, 20.9683066933067
      ), F = c(
        0.276958464357662,
        10.1206921894899, 4.18462326063615, NA
      ), p = c(
        0.759564473545402,
        0.00287422991075648, 0.0225724417916787, NA
      ), Omega2_partial = c(
        -0.0332021359560262,
        ...
        0.144188328446949, NA
      ), Eta2_CI_low = c(
        0, 0.0340186033636136,
        0.00261502598232032, NA
      ), Eta2_CI_high = c(
        0.0601580764638478,
        0.345007449145815, 0.296851216509945, NA
      ), Epsilon2 = c(
        -0.024913628761442,
      ...
      )), row.names = c(NA, -4L), class = "data.frame", ci = 0.89, model_class = c(
        "anova",
        "data.frame"
      ), digits = 2, ci_digits = 2, p_digits = 3),
      tolerance = 0.001
