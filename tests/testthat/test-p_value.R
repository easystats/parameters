...
test_that("p_value sandwich", {
  requiet("sandwich")
  mod <- lm(mpg ~ hp + factor(cyl), data = mtcars)

  # manual
  co <- coef(mod)
  se <- sqrt(diag(sandwich::vcovHC(mod, type = "HC2")))
  tstat <- co / se
  p <- 2 * stats::pt(abs(tstat),
                     df = degrees_of_freedom(mod),
                     lower.tail = FALSE)

  # automatic
  p1 <- p_value(mod, vcov_estimation = "HC2")$p
  p2 <- p_value(mod, vcov_estimation = sandwich::vcovHC, vcov_args = list(type = "HC2"))$p
  expect_equal(p1, p, ignore_attr = TRUE)
  expect_equal(p2, p, ignore_attr = TRUE)

  # pre-computed SE data.frame
  se <- standard_error(mod, vcov_estimation = "HC2")
  p3 <- p_value(mod, vcov_estimation = se)$p
  expect_equal(p3, p, ignore_attr = TRUE)
})
